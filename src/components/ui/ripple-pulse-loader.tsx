  ...
  return (
    <div className={cn("flex items-center justify-center min-h-[140px] w-full", className)}>
      <div className="relative flex items-center justify-center">
        {/* Ripple Rings - White Aesthetic */}
        <div className={cn(
          "absolute rounded-[2.5rem] border-2 border-zinc-100 bg-white/50 backdrop-blur-sm animate-ripple shadow-2xl shadow-zinc-200/50",
          size === "sm" ? "w-12 h-12" : size === "md" ? "w-24 h-24" : "w-36 h-36"
        )} />
        
        <div className={cn(
          "absolute rounded-[2rem] border-2 border-zinc-50 bg-white/30 backdrop-blur-sm animate-ripple opacity-50",
          size === "sm" ? "w-10 h-10" : size === "md" ? "w-20 h-20" : "w-32 h-32"
        )} style={{ animationDelay: "0.5s" }} />

        {/* Core Logo Box */}
        <div className={cn(
          "relative z-10 bg-white rounded-[1.5rem] shadow-xl flex items-center justify-center border border-zinc-100 transition-all duration-500",
          sizeClasses[size]
        )}>
          <svg
